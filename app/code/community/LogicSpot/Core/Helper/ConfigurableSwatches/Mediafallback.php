<?php
class LogicSpot_Core_Helper_ConfigurableSwatches_Mediafallback extends Mage_ConfigurableSwatches_Helper_Mediafallback {
	public function attachConfigurableProductChildrenAttributeMapping(array $parentProducts, $storeId)
	{
        ...
        // Normalize keys while building the array, not on every inner loop
		$optionLabels = array();
		foreach ($configAttributes as $attribute) {
			$optionLabels += array_map(function ($value) {
				return array_map('Mage_ConfigurableSwatches_Helper_Data::normalizeKey', $value);
			}, $attribute->getOptionLabels());
		}
